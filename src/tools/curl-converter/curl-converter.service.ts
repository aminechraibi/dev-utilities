...
function tokenize(cmd: string): string[] {
  ...
  while (i < cleaned.length) {
    ...
    if (cleaned[i] === '"') {
      ...
      let s = '';
      i++;
      while (i < cleaned.length && cleaned[i] !== "'") { s += cleaned[i]; i++; }
      i++;
      tokens.push(s);
    }
    else {
      let s = '';
      while (i < cleaned.length && !/\s/.test(cleaned[i]!)) { s += cleaned[i]; i++; }
      tokens.push(s);
    }
  }
  return tokens;
...
export function toFetch(p: ParsedCurl): string {
  const parts: string[] = [];
  if (p.method !== 'GET') parts.push(`  method: '${p.method}'`);
  const hs = hLines(p.headers, '    ');
  if (hs) parts.push(`  headers: {\n${hs}\n  }`);
  if (p.body != null) parts.push(`  body: ${JSON.stringify(p.body)}`);
...
export function toAxios(p: ParsedCurl): string {
  ...
  const hEntries = Object.entries(p.headers);
  const hStr = hLines(p.headers, '    ');
  const headersPart = hStr ? `  headers: {\n${hStr}\n  }` : '';

  let bodyVal = '';
  if (p.body != null) {
    try { JSON.parse(p.body); bodyVal = p.body; }
    catch { bodyVal = JSON.stringify(p.body); }
  }
...
export function toPython(p: ParsedCurl): string {
  ...
  if (entries.length) args.push('headers=headers');
  if (p.body != null) {
    try { JSON.parse(p.body); lines.push(`json_data = ${p.body}`, ''); args.push('json=json_data'); }
    catch { lines.push(`data = ${JSON.stringify(p.body)}`, ''); args.push('data=data'); }
  }
