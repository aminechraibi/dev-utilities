...
const ratio = computed(() => {
  if (!isValidHex(fg.value) || !isValidHex(bg.value)) return null;
  return getContrastRatio(fg.value, bg.value);
});

const levels = computed(() => ratio.value === null ? null : ({
      ...
      <div flex gap-6 flex-wrap>
        <div flex flex-col gap-2>
          ...
          <div flex gap-2 items-center>
            <input type="color" v-model="fg" style="width:40px;height:40px;border-radius:6px;cursor:pointer;border:none;" />
        ...
        <div flex flex-col gap-2>
          ...
          <div flex gap-2 items-center>
            <input type="color" v-model="bg" style="width:40px;height:40px;border-radius:6px;cursor:pointer;border:none;" />
    ...
    <template v-if="ratio !== null && levels !== null">
      <c-card title="Contrast ratio">
        ...
        <div flex gap-2 flex-wrap>
          <n-tag :type="levels.normalAA ? 'success' : 'error'">Normal AA {{ levels.normalAA ? '✓' : '✗' }}</n-tag>
          <n-tag :type="levels.normalAAA ? 'success' : 'error'">Normal AAA {{ levels.normalAAA ? '✓' : '✗' }}</n-tag>
          <n-tag :type="levels.largeAA ? 'success' : 'error'">Large AA {{ levels.largeAA ? '✓' : '✗' }}</n-tag>
          <n-tag :type="levels.largeAAA ? 'success' : 'error'">Large AAA {{ levels.largeAAA ? '✓' : '✗' }}</n-tag>
        </div>
      </c-card>

      <c-card title="Preview">
        <div :style="`background:${bg};color:${fg};padding:20px;border-radius:8px;`">
          <p style="font-size:16px;margin:0 0 12px;">Normal text — The quick brown fox jumps over the lazy dog.</p>
          <p style="font-size:24px;font-weight:bold;margin:0;">Large text — The quick brown fox.</p>
        </div>
      </c-card>
    </template>
  </div>
