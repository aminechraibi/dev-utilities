...
function hLines(kvs: KV[], indent: string): string {
  return activeKV(kvs).map(([k, v]) => `${indent}'${k}': '${v}'`).join(',\n');
}

function bodyValue(cfg: RequestConfig): string | null {
  if (cfg.bodyType === 'none') return null;
  if (cfg.bodyType === 'json') return cfg.bodyJson.trim() || null;
  if (cfg.bodyType === 'text') return cfg.bodyText || null;
...
export function toCurl(cfg: RequestConfig): string {
  ...
  for (const [k, v] of hEntries) lines.push(`  -H '${k}: ${v}'`);
  ...
  if (cfg.bodyType === 'json' && cfg.bodyJson.trim()) {
    ...
    const pairs = activeKV(cfg.bodyForm);
    for (const [k, v] of pairs) lines.push(`  -F '${k}=${v}'`);
  }
...
export function toFetch(cfg: RequestConfig): string {
  ...
  if (cfg.method !== 'GET') parts.push(`  method: '${cfg.method}'`);

  const allHeaders: Record<string, string> = { ...headersRecord(cfg.headers) };
  if (cfg.bodyType === 'json' && cfg.bodyJson.trim()) allHeaders['Content-Type'] = 'application/json';
  else if (cfg.bodyType === 'form') allHeaders['Content-Type'] = 'application/x-www-form-urlencoded';
...
export function toPython(cfg: RequestConfig): string {
  ...
  if (hEntries.length) {
    lines.push('headers = {');
    for (const [k, v] of hEntries) lines.push(`    '${k}': '${v}',`);
    lines.push('}', '');
  }

  const method = cfg.method.toLowerCase();
  const args: string[] = [JSON.stringify(url)];
  if (hEntries.length) args.push('headers=headers');
...
export function toPhpCurl(cfg: RequestConfig): string {
  ...
  if (hEntries.length || cfg.bodyType === 'json') {
    if (cfg.bodyType === 'json') allHeaders['Content-Type'] = 'application/json';
    const allH = Object.entries(allHeaders);
    lines.push('');
    lines.push('$headers = [');
    for (const [k, v] of allH) lines.push(`    '${k}: ${v}',`);
    lines.push('];');
...
export function toGo(cfg: RequestConfig): string {
  ...
  const body = bodyValue(cfg);
  const allHeaders: Record<string, string> = { ...headersRecord(cfg.headers) };
  if (cfg.bodyType === 'json' && body) allHeaders['Content-Type'] = 'application/json';
